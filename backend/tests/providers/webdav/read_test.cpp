...
static ph::HttpTransportFn fx(){return[](const ph::HttpTransportRequest& r)->ph::HttpTransportResponse{ph::HttpTransportResponse o;if(r.method=="PROPFIND"){o.status=405;}else if(r.range_start.has_value()){auto s=r.range_start.value();auto e=r.range_end.value();o.status=206;auto len=std::min(e-s+1,6ULL-s);o.headers["Content-Range"]="bytes "+std::to_string(s)+"-"+std::to_string(s+len-1)+"/6";std::string b="abcdef";if(s<6){auto en=std::min(6ULL,s+len);o.body.assign(b.begin()+s,b.begin()+en);}}return o;};}
