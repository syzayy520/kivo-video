...
int main(){ph::HttpRangeSessionStore s;auto o=wd::WebDavRangeBridge::open(SourceOpenRequest{SourceUri::from_raw("http://x.com/f")},fx(),s);CHECK_TRUE(o.is_success());auto id=o.session_if_success()->session_id;auto dr=wd::WebDavRangeBridge::make_direct_play_input(id,s);CHECK_TRUE(dr.is_success());wd::WebDavRangeBridge::close(id,s);auto da=wd::WebDavRangeBridge::make_direct_play_input(id,s);CHECK_TRUE(!da.is_success());return 0;}
