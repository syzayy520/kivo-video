...
int main(){ph::HttpRangeSessionStore s;auto o=wd::WebDavRangeBridge::open(SourceOpenRequest{SourceUri::from_raw("http://x.com/path")},fx(),s);CHECK_TRUE(o.is_success());auto ev=wd::WebDavRangeBridge::get_evidence(o.session_if_success()->session_id,s);for(auto&i:ev.items){CHECK_TRUE(i.safe_description.find("http://x.com")==std::string::npos);}return 0;}
