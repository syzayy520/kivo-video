...
source_core::SourceError validate_path(const std::string& path) {
    if (path.empty()) {
        return {source_core::SourceErrorCode::invalid_request, "empty path"};
    }
    ...
    if (ec) {
        return {source_core::SourceErrorCode::not_found, "path not accessible"};
    }
    ...
    // UNC paths: reject for now (path is a std::string, check first chars)
...
source_core::SourceError probe_file(
    ...
    if (ec) {
        return {source_core::SourceErrorCode::internal_error, "filesystem error"};
    }
    ...
    if (!ec) {
        out_mtime = static_cast<std::uint64_t>(
